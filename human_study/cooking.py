...
def make_game(settings: Mapping[str, str], options: Optional[GameOptions] = None) -> textworld.Game:
    ...
    walkthrough = [_swap_words(command.format(**fake_words["actions"]), fake_words["words"]) for command in walkthrough]
    ...
    skills_uuid = "+".join("{}{}".format(k, "" if settings[k] is True else settings[k]) for k in SKILLS if k in settings and settings[k])
...
register(name="tw-cooking",
         desc="Generate games for the First TextWorld Competition",
         make=make_game,
