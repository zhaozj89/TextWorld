...
class AbstractEvent:
    ...
    def __init__(self, actions: Iterable[Action] = (), commands: Iterable[str] = (), name=None) -> None:
        """
        Args:
    ...
    def __eq__(self, other: Any) -> bool:
        return (isinstance(other, AbstractEvent) and
...
class EventCondition(AbstractEvent):
    ...
    def __hash__(self) -> int:
        return hash((self.actions, self.commands, self.condition))  # TODO: call super().__hash__ ?
...
class Event:  # For backward compatibility.
    ...
    .. warning:: Deprecated in favor of :py:class:`textworld.generator.EventCondition <textworld.generator.game.EventCondition>`.
    """
...
class EventAction(AbstractEvent):
    ...
    def is_triggering(self, state: Optional[State] = None, action: Optional[Action] = None, callback: Optional[callable] = None) -> bool:
        ...
        is_triggering = all(ph.name == mapping[ph].name for ph in self.action.placeholders if ph.name != ph.type)
        if callback and is_triggering:
            callback(self)
    ...
    def __hash__(self) -> int:
        return hash((self.actions, self.commands, self.action))  # TODO: call super().__hash__ ?
    ...
    def deserialize(cls, data: Mapping) -> "EventAction":
        ...
        Args:
            data: Serialized data with the needed information to build a
                  `EventAction` object.
        """
        #action = Action.deserialize(data["action"])
...
class EventProgression:
    ...
    def update(self, action: Optional[Action] = None, state: Optional[State] = None, callback: Optional[callable] = None) -> None:
...
class GameProgression:
    ...
    def winning_policy(self) -> Optional[List[Action]]:
        ...
        # trees = []
        # for quest in self.quest_progressions:
        #     if not quest.completable or quest.done:
        #         continue

        #     tree = ActionDependencyTree(kb=self.game.kb, element_type=ActionDependencyTreeElement,)
        #     for action in quest.winning_policy[::-1]:
        #         tree.push(action)
