...
def place_food(M, name, rng, holder=None, place_it=True):
    holder = holder or pick_location(M, FOODS[name]["locations"], rng)
...
def make(settings: Mapping[str, str], options: Optional[GameOptions] = None) -> textworld.Game:
    ...
    Notes:
        ...
        * go{1,6,9,12} : Number of locations in the game.
        ...
        Other allowed settings are:

        * split: Control which foods can be used. Can either be 'train', 'valid', 'test'.
          Default: foods from all dataset splits can be used.
        * neverending: If `True` the generate game won't have any quests.
    """
    skills = settings
    split = settings.get("split", "")
    neverending = settings.get("neverending", False)
    rngs = options.rngs
    rng_map = rngs['map']
    ...
    options.nb_rooms = skills.get("go", 6)
    if options.nb_rooms != 6:
    ...
    M.connect(kitchen.north, pantry.south)
    M.connect(kitchen.west, bedroom.east)
    kitchen_backyard = M.connect(kitchen.east, backyard.west)
    livingroom_garden = M.connect(livingroom.east, garden.west)
    ...
    # Manually define the recipe.
    nb_ingredients = skills.get("recipe", 3)
    assert nb_ingredients > 0 and nb_ingredients == 3, "recipe must have {3} ingredients."
    flour = place_food(M, "flour", rng_objects)
    red_potato = place_food(M, "red potato", rng_objects, holder=M.find_by_name("fridge"))
    yellow_onion = place_food(M, "yellow onion", rng_objects, holder=M.inventory)
    ingredient_foods = [flour, red_potato, yellow_onion]
    ...
    # Manually remove food preprations
    for preparation in itertools.product(TYPES_OF_COOKING[1:], TYPES_OF_CUTTING):
        if preparation in allowed_food_preparations["yellow onion"]:
            allowed_food_preparations["yellow onion"].remove(preparation)
    ...
    for ingredient in ingredient_foods:
        ...
        if same_prefix_list and rng_objects.rand() > 0.5:
            _place_one_distractor(same_prefix_list)

        if same_suffix_list and rng_objects.rand() > 0.5:
...
# def make_game(settings:str, options: Optional[GameOptions] = None) -> textworld.Game:
#     """ Make a Cooking game of the desired difficulty settings.

#     Arguments:
#         settings:
#             Skills used in the game (see notes).
#         options:
#             For customizing the game generation (see
#             :py:class:`textworld.GameOptions <textworld.generator.game.GameOptions>`
#             for the list of available options).
...
def build_argparser(parser=None):
    ...
    group = parser.add_argument_group('Cooking game settings')
    group.add_argument("--recipe", required=True, type=int, choices=[1, 2, 3],
                       help="How many ingredients needed for the recipe.")
    group.add_argument("--take", type=int, choices=[0, 1, 2, 3], default=0,
                       help="How many ingredients that will need to be found.")
    group.add_argument("--go", type=int, choices=[6], default=6,
                       help="The number of location in the world. Default: %(default)s.")
    group.add_argument("--open", action="store_true",
                       help="If provided, initial state of all doors and containers will be closed.")
    group.add_argument("--cook", action="store_true",
                       help="If provided, ingredients may need to be cooked before making the recipe.")
    group.add_argument("--cut", action="store_true",
                       help="If provided, ingredients may need to be cut before making the recipe.")
    group.add_argument("--drop", action="store_true",
                       help="If provided, player will have limited inventory capacity.")
    group.add_argument("--split", choices=["train", "valid", "test"], default="",
                       help="Specify the data split to use for generating the game.")
    group.add_argument("--neverending", action="store_true",
                       help="If provided, the game won't have any ending nor rewards.")
    return parser


register(name="techfest2019",
         desc="Generate a cooking game for TechFest 2019",
         make=make,
